...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use BLE instead of serial (requires bleak, ESPectre v2.7.0+; tested with v2.8.0)",
    )
...
# -- ESPectre BLE GATT UUIDs (v2.7.0+, unchanged in v2.8.0) --
...
class SharedState:
    ...
    def set_motion_detected(self, detected: bool) -> None:
        with self._lock:
...
def serial_reader_thread() -> None:
    ...
    while not shutdown_event.is_set():
        ...
        try:
            ...
            while not shutdown_event.is_set():
                ...
                if match:
                    try:
                        ...
                        state.set_motion_detected(bool(is_motion) if is_motion is not None else score >= 100.0)
            ...
            state.set_motion_detected(False)
...
# =========================================================================== #
#  任務一 (BLE 版)：透過藍芽接收 ESPectre 資料 (ESPectre v2.7.0+ / v2.8.0)
# =========================================================================== #
...
async def _ble_reader_async() -> None:
    ...
    def on_telemetry(sender, data: bytearray) -> None:
        ...
        if len(data) >= 8:
            ...
            motion_detected = movement >= threshold
            state.set_movement_score(score)
            state.set_motion_detected(motion_detected)
            ...
            if settings.get("algorithm") == "ml":
                ...
                state.set_fall_detected(score >= 100.0 and motion_detected)
            else:
                state.set_fall_detected(fall_detector.update(score, motion_detected))
            logger.info("[BLE] mvmt=%.4f thr=%.4f score=%.1f%% motion=%s",
                        movement, threshold, score, motion_detected)
...
def simulated_serial_thread() -> None:
    ...
    while not shutdown_event.is_set():
        ...
        state.set_motion_detected(score >= 15.0)
                ...
                if settings.get("thresholdMode") == "manual" and settings.get("manualThreshold") is not None:
                    try:
                        ...
                        if not math.isfinite(threshold) or threshold < 0 or threshold > 10:
                            raise ValueError("threshold must be in 0.0-10.0")
...
def build_broadcast_payload() -> str:
    ...
    motion_detected = state.is_motion_detected()
...
def main() -> None:
    ...
    if SIMULATE_MODE:
        ...
        print("  ** BLE MODE (ESPectre v2.7.0+ / v2.8.0, wireless) **")
    print("=" * 60)
    print()
